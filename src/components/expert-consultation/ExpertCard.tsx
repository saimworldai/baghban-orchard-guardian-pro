    ...
    >
      <Card className="overflow-hidden hover:shadow-lg transition-shadow border-primary/10">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {expert.languages.map((lang) => (
                  <Badge key={lang} variant="secondary" className="text-xs bg-primary/5">
                    {lang}
                  </Badge>
                ))}
              </div>
              {expert.available ? (
                <Badge className="bg-green-100 text-green-800 border-green-200 mt-1">Available Now</Badge>
              ) : (
                <Badge variant="outline" className="text-gray-500 mt-1">Unavailable</Badge>
              )}
            </div>
          </div>
        </CardContent>
        <CardFooter className="flex gap-2 bg-gray-50/50 mt-4 p-3">
          ...
          <Button variant="outline" size="sm" className="flex-1 hover:bg-blue-50 hover:text-blue-700">
